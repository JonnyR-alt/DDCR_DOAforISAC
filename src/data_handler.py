...
def read_data(path: str):
    ...
    # Allow loading serialized objects containing Samples.
    # PyTorch >=2.6 defaults weights_only=True; older versions don't have add_safe_globals.
    if add_safe_globals is not None:
        add_safe_globals([Samples])
    try:
        data = torch.load(path, weights_only=False)
    except TypeError:
        # Older PyTorch: torch.load doesn't accept weights_only
...
def set_dataset_filename(system_model_params: SystemModelParams, samples_size: float):
    ...
    # 动态追加 doa_gap
    if hasattr(system_model_params, 'doa_gap') and system_model_params.doa_gap != 15.0:
        suffix_filename += f"doa_gap={system_model_params.doa_gap}_"
        
    # 动态追加 fixed_gap 标记
    if getattr(system_model_params, 'fixed_gap', False):
        suffix_filename += "fixed_"
