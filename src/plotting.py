...
def plot_root_music_spectrum(roots: np.ndarray, predictions: np.ndarray,
    ...
    """
    # If roots are missing (e.g., diff_method="esprit" or method didn't return them), skip plotting
    if roots is None or predictions is None:
      return
    # Initialize figure
    plt.style.use('default')
    ...
    Path("data/spectrums").mkdir(parents=True, exist_ok=True)
