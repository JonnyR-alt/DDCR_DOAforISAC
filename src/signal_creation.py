...
class Samples(SystemModel):
    ...
    def set_doa(self, doa, doa_gap: float = 15.0):
        """
        ...
        -----
            ...
            doa_gap (float): Minimum gap between generated DOAs when doa is None.
        """
        ...
        def create_doa_with_fixed_gap(gap: float):
            """Create M angles with EXACTLY the given gap between them."""
            M = self.params.M
            while True:
                # Pick a starting angle
                start_angle = np.random.rand() * (180 - (M - 1) * gap) - 90
                # Generate exact sequence
                DOA = np.round(np.array([start_angle + i * gap for i in range(M)]), decimals=2)
                # Ensure they stay in [-90, 90] bounds naturally
                if np.all(DOA >= -90.0) and np.all(DOA <= 90.0):
                    break
            return DOA

        if doa == None:
            # Check if strictly fixed gap is requested
            is_fixed = getattr(self.params, 'fixed_gap', False)
            if is_fixed:
                self.doa = np.array(create_doa_with_fixed_gap(gap=doa_gap)) * D2R
            else:
                # Generate angels with gap greater than minimum gap (nominal case)
                self.doa = np.array(create_doa_with_gap(gap=doa_gap)) * D2R
        else:
            # Generate
    ...
    def signal_creation(self, signal_mean: float = 0, signal_variance: float = 1):
        ...
        signal_type = str(self.params.signal_type)
        signal_type_l = signal_type.lower()
        # NarrowBand signal creation
        if signal_type_l.startswith("narrowband"):
        ...
        elif signal_type_l.startswith("broadband"):
